...
<?php
   ...
   while( $row=$result->fetch_assoc()){
    if($PW!=$row["password"]){
        echo '<head><meta http-equiv="refresh" content="1; url=SLI.html" /><h1>Please enter a Correct Password</h1></head>';
    }
    if($row['category']!=2){
         echo '<head><meta http-equiv="refresh" content="1; url=index.html" /><h1>Please Select the correct category</h1></head>';
    }
    if($PW==$row["password"] and $row['category']==2){
      echo '<head><meta http-equiv="refresh" content="1; url=superresp.php?UN='.$UN.'&N='.$row["$name"].'&R='.$row[$roll].'"/>';
    }
    }
?>
